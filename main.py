...
def main():
    ...
    activities = ['Input Tickers', "Input tickers as CSV"]
    choice = st.sidebar.selectbox("Select Activities", activities)

    if choice == 'Input Tickers':

        st.subheader("*Revolut Daily Stock changes*")
        no_days = st.slider(
            'How many last days News to do sentiment analysis?', 1, 7, step=1)
        threshold = st.slider(
            'Threshold sentiment stocks to be selected', 0.0, 1.0, step=0.1)
        st.write('*Input tickers seperated by Comma without spaces:*')
        st.write('Ex: TSLA,MSFT,AAPL,SQ **not** TSLA, MSFT, AAPL, SQ')
        tick = st.text_input('')

        if tick:

            stock_list = tick.split(",")
            ...
            mean_scores = parsed_and_scored_news.groupby(
                ['ticker', 'date']).mean()
            ...
            st.write(
                'Stocks data of Sentiment greater than **{}**'.format(threshold))
            st.dataframe(df)
            ...
            def to_excel(df):
                ...
                df.to_excel(writer, index=False,
                            sheet_name='Sheet1', float_format="%.2f")
                writer.save()
            ...
            def get_table_download_link(df):
                ...
                # decode b'abc' => abc
                return f'<a href="data:application/octet-stream;base64,{b64.decode()}" download="Your_File.xlsx">Download file</a>'
            st.write('**Download the Positive Sentiment Stocks**')
            df.reset_index(level=df.index.names, inplace=True)
            st.markdown(get_table_download_link(df), unsafe_allow_html=True)

    if choice == 'Input tickers as CSV':
        st.subheader("*Daily Stock changes*")
        no_days = st.slider(
            'How many past days News to do sentiment analysis?', 1, 7, step=1)
        threshold = st.slider(
            'Threshold sentiment stocks to be selected', 0.0, 1.0, step=0.1)
        st.write('*Input CSV file contains column namely **Ticker**:*')
        ...
        if data is not None:
                ...
                df = df.apply(pd.to_numeric, errors='coerce')
                st.write('After Data Wrangling')
                df = df.sort_values(['sentiment', 'Volume', 'Change'], ascending=[
                                    False, False, True])
                st.dataframe(df)
